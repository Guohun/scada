...
{
	int i,j,k,t_1=0,t_2=0;//now_che;
	char *s1,*s2;
	char zoj[20];
	char temp[40];
	char th1_dh[6],th2_dh[6],yz1_dh[6],yz2_dh[6];
	char D_sx[5][6];
//	struct  elect_send_type     elec_input;  //elec
//	struct  elect_send_type     elec_output;  //elec
	 HDC  hdc;
...
//			TextOut(hdc,50,80,temp,4);
	 ...
	 float jt;
	//	read_run(&run_table_do, &run_flag);
	 if (((run_flag==1 || run_flag==2) && cs_flag==0)||(run_flag==3))
       {
		read_produce(&p_y );
		init_elec_input(&elec_input);
		init_elec_output(&elec_output);
...
//			   TextOut(hdc,20,150,temp,x);
			if(p_y.th1[i].weight>0.0)
			{
				...
				head_d(d_1,"D",j,5);
				strcat(d_1,ii_bin((int)(p_y.th1[i].weight*10),zoj));	//PLC中16进制改为10进制把ii_bin改成h16_16a
			//	strcat(d_1,ii_bin(0,zoj));
...
/*		for(i=0;i<4;i++)
	...
	//		TextOut(hdc,300,80,temp,4);
			...
			for(j=0;j<16;j++)
				if((d_d[2]&(short)pow(2,j))>0)
				elec_input.data[i*32+j+16]=1;
				else
				elec_input.data[i*32+j+16]=0;
		//	for(j=0;j<16;j++)
		//	{
		//	sprintf(temp,"%4d",elec_input.data[i*32+j]);
		...
		for(i=0;i<6;i++)
		{
			...
			for(j=0;j<16;j++)
				if((d_d[1]&(short)pow(2,j))>0)
				elec_output.data[i*32+j]=1;
				else
				elec_output.data[i*32+j]=0;
			for(j=0;j<16;j++)
				if((d_d[2]&(short)pow(2,j))>0)
				elec_output.data[i*32+j+16]=1;
				else
				elec_output.data[i*32+j+16]=0;
		}
		//		sprintf(temp,"%4x",d_d[1]);
		for(i=0;i<13;i++)
		{
