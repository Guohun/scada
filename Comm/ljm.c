...
*******************************************************************/
	 ...
	 Init_Font(72,"宋体");
	write_multi_screen(50,30,temp);
	Sleep(30);
	Init_Font(18,"宋体");
    run_table_do.now_che=1;
    write_run(&run_table_do,-1);
	 ...
	 Sleep(550);
	    i=0;jt=1;
		 ...
		 while(i<p_y.pei_max)
		 {
			...
			write_multi_screen(50,320,temp);
			Sleep(100);
...
LRESULT WINAPI Read_Elec_Thread (int x)
{
	 ...
	 do{
		...
		if(elec_output.flag==2)
				sprintf(temp,"输入信号总长度=%d 输出信号总长度=%d 状态=调试",
				   elec_input.length,elec_output.length);
		else
				sprintf(temp,"输入信号总长度=%d 输出信号总长度=%d 状态=正常",
				   elec_input.length,elec_output.length);				
			   x=strlen(temp);
               TextOut(hdc,20,1,temp,x);				
			   write_multi_screen(1,20,temp);
...
/*				for(i=0;i<elec_input.length;i++)
				{
					sprintf(temp,"elecin[%d].node_name=%d,elecout[%d].node_name=%d",
					   i,elec_input.note_name[i],i,elec_output.note_name[i]);
						x=strlen(temp);
		               TextOut(hdc,20,j,temp,x);				
					   write_multi_screen(j,20,temp);
					   Sleep(100);
					   j=j+20;
				}
*/				
					x=Read_VbKey();
				if(x==0)				
					sprintf(temp,"碳黑设备Max=%f min=%f 没读入键盘",
							my_send.max,my_send.min);
				
				else
					sprintf(temp,"碳黑设备Max=%f min=%f  读入键盘=%4d",
							my_send.max,my_send.min,x);
				
			   x=strlen(temp);
               TextOut(hdc,120,20,temp,x);													
			   write_multi_screen(20,120,temp);
			   Sleep(80);
				elec_number=30;
				Get_Elec_Check_Table(my_elec,&elec_number);
/*				for(x=0;x<elec_number;x++)
				{
					   sprintf(temp,"name=%d str=%s run=%d,run_set=%d, check=%d, check_set=%d use=%d",
						   my_elec[x].name,my_elec[x].str,my_elec[x].run,my_elec[x].run_set,
						   my_elec[x].check,my_elec[x].check_set,my_elec[x].use_flag);												
						j=j+20;
    ...
    do{
       ...
       {			   	
		   ...
		   sprintf(temp,"碳投1=%2d,碳投2=%2d,油投1=%2d,油投2=%2d,胶1数=%2d,胶2数=%2d,总段数=%2d",
				...
				write_multi_screen(100,20,temp);
				Sleep(100);
